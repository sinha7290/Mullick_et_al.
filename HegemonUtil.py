...
#  def getSource(self):
    # Ensure that missing `source` defaults to `name` or a fallback value
#    if 'source' in self.hash:
#        return self.hash['source']
#    return self.hash.get('name', 'unknown_source')  # Fallback to `name` or `unknown_source`
...
#def getSource(self):
    # Check if `hash` contains the `source` key; otherwise, use a fallback
#    if hasattr(self.rdataset, 'hash') and 'source' in self.rdataset.hash:
#        return self.rdataset.hash['source']
#    return self.rdataset.hash.get('name', 'unknown_source')  # Default to 'name' or 'unknown_source'
